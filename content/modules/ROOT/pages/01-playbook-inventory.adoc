...
[TIP]
====
The Ansible extension and its automation coding assistant are pre-configured in this lab to use a *LiteMaaS* model endpoint -- Red Hat AI's Model-as-a-Service platform. The configuration is stored in VS Code's `settings.json` (both at the user level and in the workspace's `.vscode/settings.json`). You do not need to log in, authenticate, or provide any credentials; everything is already set up for you.
